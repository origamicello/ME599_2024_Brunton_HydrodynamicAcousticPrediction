...
subplot_titles = {'Inlet', 'Top Wall'};

% Create a figure with a larger size
figure('Position', [100, 50, 1200, 800]); % Adjust position and size as needed

% Add an overall title to the figure
sgtitle('500Hz Impulse ERA Overlay', 'FontSize', 16, 'FontWeight', 'bold');
...
% Indices for the outputs to plot
plot_indices = [1, 3];

% Iterate over each specified output and plot its impulse response
for idx = 1:2
    i = plot_indices(idx);
    ...
    % Plot the impulse response for the current output
    subplot('Position', subplot_positions(idx, :));
    plot(y_actual, 'LineWidth', 2);
    hold on;
    plot(y_predicted, '--', 'Color', [0.8, 0.6, 0.0], 'LineWidth', 2); % Dark yellow color
    grid on;
    title(subplot_titles{idx}, 'FontSize', 12, 'FontWeight', 'bold');
    legend(['Model (R² = ', num2str(r_squared, '%.2f'), ')'], ['ERA (', num2str(elapsed_time, '%.2f'), 's)'], 'FontSize', 12);
